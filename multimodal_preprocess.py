...
def get_sinusoid_encoding_table(n_position, d_hid):
    ...
    return Tensor(sinusoid_table).unsqueeze(0)
...
def interpolate_pos_encoding(
    ...
    return ops.cat((class_emb, pos_embed), axis=1)
...
class PatchEmbedGeneric(nn.Cell):
    """
    PatchEmbed from Hydra
    """

    def __init__(self, proj_stem, norm_layer: Optional[nn.Cell] = None):
        super().__init__()

        if len(proj_stem) > 1:
            self.proj = nn.SequentialCell(*proj_stem)
        else:
...
class RGBDTPreprocessor(VerboseNNModule):
    ...
    def construct(self, vision=None, depth=None, patch_mask=None):
...
class AudioPreprocessor(RGBDTPreprocessor):
    ...
    def construct(self, audio=None):
        return super().construct(vision=audio)
...
class ThermalPreprocessor(RGBDTPreprocessor):
    ...
    def construct(self, thermal=None):
        return super().construct(vision=thermal)
...
class IMUPreprocessor(VerboseNNModule):
    ...
    def tokenize_input_and_cls_pos(self, input, stem):
        ...
        if self.num_cls_tokens > 0:
            ...
            tokens = ops.cat((class_tokens, tokens), aixs=1)
        if self.use_pos_embed:
            tokens = tokens + self.pos_embed
        return tokens

    def construct(self, imu):
        # Patchify
        imu = imu.unfold(
            -1,
            self.kernel_size,
